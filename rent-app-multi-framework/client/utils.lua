function ShowNotification(message, type, duration)
    ...
    if Config.NotifySystem == 'codem-phone' then
        -- Codem-phone notification fallback to ox_lib
        -- Note: codem-phone doesn't have SendNotification export
        -- Using ox_lib as fallback for better compatibility
        lib.notify({
            title = 'Line Rental',
            description = message,
            type = type,
    ...
    elseif Config.NotifySystem == 'esx' then
        -- ESX notify (direct export call, independent of framework)
        local success, ESX = pcall(function()
            return exports['es_extended']:getSharedObject()
        end)

        if success and ESX and ESX.ShowNotification then
            ESX.ShowNotification(message)
        else
            -- Silent fallback to ox_lib if ESX not available
            lib.notify({
                description = message,
                type = type,
                duration = duration,
    ...
    elseif Config.NotifySystem == 'qb' then
        -- QBCore notify (direct export call, independent of framework)
        local success, QBCore = pcall(function()
            return exports['qb-core']:GetCoreObject()
        end)

        if success and QBCore and QBCore.Functions and QBCore.Functions.Notify then
            QBCore.Functions.Notify(message, type, duration)
        else
            -- Silent fallback to ox_lib if QB not available
            lib.notify({
                description = message,
                type = type,
                duration = duration,
